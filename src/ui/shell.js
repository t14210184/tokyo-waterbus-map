/**
 * Main UI Shell Component for Tokyo Waterbus Atlas (Phase 0 Truthful Traveller Foundation v1.1.0-RC.3.22)
 */
    ...
    <div class="app-shell">
      ...
      <header class="app-header">
        <div class="brand-area">
          ...
          <div>
            <div class="brand-title" style="display: flex; align-items: center; gap: 0.5rem;">
              ...
              <span class="badge" style="font-size: 0.65rem; background: rgba(255, 255, 255, 0.08); color: #94a3b8; border: 1px solid rgba(255,255,255,0.15); cursor: not-allowed;" title="Multilingual UI support planned for Phase 1">
                Language: 繁中 (coming soon)
              </span>
            </div>
            <div class="brand-subtitle">
              Official service status, timetable guidance, pier finder & traveller reference
            </div>
          </div>
        </div>
        ...
        <div style="display: flex; align-items: center; gap: 0.75rem;">
          ...
          <button id="btn-offline-demo" class="btn btn-secondary" aria-label="啟動離線示範" aria-pressed="false" title="開啟概念離線示範動畫" style="padding: 0.35rem 0.65rem; min-height: 32px; font-size: 0.75rem; background: rgba(56, 189, 248, 0.12); color: #38bdf8; border: 1px solid rgba(56, 189, 248, 0.4);">
            ▶ 啟動離線示範
          </button>
      ...
      <!-- Offline Demo Disclaimer Banner (P0-4) -->
      <div id="demo-disclaimer-banner" style="display: none; background: rgba(15, 23, 42, 0.95); border-bottom: 1px solid rgba(56, 189, 248, 0.4); padding: 0.5rem 1rem; font-size: 0.73rem; color: #38bdf8; align-items: center; justify-content: space-between; gap: 1rem;">
        <div>
          <strong>離線示範：非 GPS/AIS、非即時船位、非當日班次。</strong> 航線為概略參考，不可用於導航或安全判斷。
          <button id="btn-toggle-data-levels" class="btn btn-secondary" style="font-size: 0.68rem; padding: 0.15rem 0.4rem; margin-left: 0.5rem; text-decoration: underline;">了解資料層級</button>
        </div>
        <div style="display: flex; gap: 0.5rem; flex-shrink: 0;">
          <button id="btn-reset-demo" class="btn btn-secondary" style="padding: 0.25rem 0.55rem; min-height: 28px; font-size: 0.7rem;">重設示範</button>
          <button id="btn-stop-demo" class="btn btn-secondary" style="padding: 0.25rem 0.55rem; min-height: 28px; font-size: 0.7rem; background: #ef4444; color: #fff; border: none;">停止示範</button>
        </div>
      </div>

      <!-- Data Trust Levels Info Drawer (P0-4 Expandable) -->
      <div id="data-levels-modal" style="display: none; background: rgba(7, 25, 35, 0.96); border-bottom: 1px solid rgba(56, 189, 248, 0.3); padding: 0.75rem 1.25rem; font-size: 0.75rem; color: #cbd5e1;">
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 0.4rem;">
          <strong style="color: #38bdf8; font-size: 0.85rem;">📊 Data Trust Model (資料可信度層級說明)</strong>
          <button id="btn-close-data-levels" class="btn btn-secondary" style="padding: 0.15rem 0.4rem; font-size: 0.7rem;">關閉 ×</button>
        </div>
      ...
      <div class="app-body">
        ...
        <aside class="control-sidebar" id="control-sidebar">
          ...
          <!-- Navigation Tabs in Traveller-First Order (P0-2) -->
          <nav class="sidebar-tabs" role="tablist">
            <button class="tab-btn active" data-tab="today" role="tab" aria-selected="true" aria-label="今天狀態">
              <span class="tab-icon">${ICONS.ship}</span>
              <span>今天狀態</span>
            </button>
            <button class="tab-btn" data-tab="routes" role="tab" aria-selected="false" aria-label="航線">
              <span class="tab-icon">${ICONS.map}</span>
              <span>航線</span>
            </button>
            <button class="tab-btn" data-tab="piers" role="tab" aria-selected="false" aria-label="碼頭">
              <span class="tab-icon">${ICONS.pier}</span>
              <span>碼頭</span>
            </button>
            <button class="tab-btn" data-tab="planner" role="tab" aria-selected="false" aria-label="行程規劃">
              <span class="tab-icon">${ICONS.compass}</span>
              <span>行程規劃</span>
            </button>
            <button class="tab-btn" data-tab="guide" role="tab" aria-selected="false" aria-label="攻略">
              <span class="tab-icon">${ICONS.data}</span>
              <span>攻略</span>
            </button>
            <button class="tab-btn" data-tab="explore" role="tab" aria-selected="false" aria-label="探索">
              <span class="tab-icon">${ICONS.compass}</span>
              <span>探索</span>
            </button>
          </nav>
      ...
      <!-- Footer Status Gateway Bar & Build Identity Disclosure (P0-1 & P0-2 Secondary Entry) -->
